...
<link rel="stylesheet" href="https://sightseeingscanner.com/css/datepicker.css">
<style>
  .alert { 
  ...
  @foreach ($iacs->subjects_infos as $key => $item)
    ...
    @php
      ...
      foreach ($iacs->subjects_infos as $subj){
        $subj_ids[] =!empty( $subj->student_subjects_infos->where('student_id', auth()->user()->student_id)->first()->subjects_info_id) ? $subj->student_subjects_infos->where('student_id', auth()->user()->student_id)->first()->subjects_info_id :''; 
      }
      $implodes = implode(',',$subj_ids);
    @endphp
<!--     $item->student_subjects_infos->where('student_id', auth()->user()->student_id)->first()->id -->
  <div class="col-md-12"> 
    <a href='#' data-toggle="modal" data-target="#changeTimeModal" data-cl="{{$implodes}}" data-student='{{auth()->user()->student_id}}' data-time="{{!empty($item->student_subjects_infos->where('student_id', auth()->user()->student_id)->first()->time_slot->id) ? $item->student_subjects_infos->where('student_id', auth()->user()->student_id)->first()->time_slot->id : ''}}" class='changeClassTime'><div class='on-hove  '> <i class='fa fa-edit'> Change Class Timings</i>  </div></a> 
...
<div class="card-box class_resources">
  ...
  <div class=" d-flex align-items-center justify-content-center flex-wrap">
    <a class="btn  py-2 mx-2  mw-220 text-center text-white getnotification" style='background:linear-gradient(45deg,#8a2be2e8, #c8beff);' data-toggle='modal' data-target='#notificationModal'>Class Notifications <span class='totalNotify'>{{$notifications ? '('.$notifications.')' :'(0)'}}</span></a>
    ...
    <a class="btn pink-gradient py-2 mx-2  mw-220 text-center text-white"
      href="{{route('student.assignments', request()->iacs_id)}}">Assignments <span class='assignmentnotifications'>{{!empty($assignmentnotifications) ? '('.$assignmentnotifications.')' :'(0)'}}</span></a>
    <a class="btn blue-gradient py-2 mx-2  mw-220 text-center text-white"
      href="{{route('student.doubts', request()->iacs_id)}}">Doubts <span class='dnotifications'>{{$dnotifications ? '('.$dnotifications.')' :'(0)'}}</span></a>
    <a class="btn orange-gradient py-2 mx-2  mw-220 text-center text-white"
      href="{{route('student.tests', request()->iacs_id)}}">Tests <span class='testsnotification'>{{$testsnotification ? '('.$testsnotification.')' :'(0)'}}</span></a>
    <a class="btn sea-gradient py-2 mx-2  mw-220 text-center text-white"
      href="{{route('student.extra-classes', request()->iacs_id)}}">Extra Classes <span class='extranotifications'>{{$extranotifications ? '('.$extranotifications.')' :'(0)'}}</span></a> 
  </div>
</div>
...
'test')->orderByDesc('id')->take(10)->get();
  ...
  <div class="col-xl-12 col-md-12">
    ...
    <div class="row m-0 ">
      ...
      <div class="col-xl-3 ml-auto pr-0">
        ...
        request()->iacs_id)->whereDate('lecture_date', '<', date('Y-m-d 00:00:00'))->get();  
          if($lectures->count() > 0){
