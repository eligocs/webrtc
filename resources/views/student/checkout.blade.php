    ...
    <div class="col-md-10">
      ...
      <div class="row card-box brder bg-board align-items-center">
        <div class="col-lg-12 mb-md-2 poisition-relative">
          <h3 class="heading-title mt-0 mb-0 text-center heading text-white donis_font">{{$class->name}}</h3>
            ...
            <div class="col-md-4 p-1">
              ...
              <a href="{{route('student.subject_detail', $iacs_id)}}" class="d-flex flex-column w-100 h-100">
                <span class="{{$colors[$key%6]}} br-6 px-2 py-3 h-140">
                  <h4 class=" text-left fw-100 text-white mt-0 mb-2 m-auto text-center">
                    <b>{{$institute_assigned_class_subject->subject->name}}</b>
                  </h4>
                  @php
                  try {
                  $date_of_days = [];
        ...
        <div class="col-md-5 pb-md-5 pl-md-4 mb-md-5" style="z-index: 99999;">
          ...
          {{-- <h4 class=" mt-4 mb-0 text-center fw-100 text-white position_absl">Your Overall Performance</h4> --}}
          <div class="postn_absolute">
            ...
            @php
            $session_key = 'student_' . auth()->user()->student_id . '_class_' . $data['class_id'];
            @endphp
            <div style="margin-top: -55px;">

              @if(isset($isfreetrial))

              @else
              <h5 class="text-white">Total Amount - <span style="float:right">Rs.
                  {{number_format($class->price)}}/-</span>
              </h5>
              @endif

              @if (request()->code)
              <h5 class="text-white">Update Amount - <span style="float:right">Rs.
              ...
              <form action="" id="couponForm">
                @csrf
                <label for="" class="text-white">Have a Coupon?</label>
                <br>
                <input type="hidden" custom="Hidden Element" name="form_data" value="{{$_SESSION[$session_key]}}">
                <input type="text" id="coupon" name="coupon" placeholder="Enter Coupon Code" value="{{$_GET['code'] ?? ''}}" required>
                <button type="submit">Apply</button>
              </form>
              @endif
              <form action="{{route('student.pay')}}" method="post">
                @csrf
                <br>
                <input type="hidden" custom="Hidden Element" name="form_data" value="{{$_SESSION[$session_key]}}">
                <input type="hidden" custom="Hidden Element" name="mode_of_class" value="{{$_SESSION[$session_key]}}">
                @if(isset($isfreetrial) && $isfreetrial == true)
                <input type="hidden" name="free_trial" value="yes">
                <input type="submit" class="razorpay-payment-button" value="Start Free Trial">
                @else
