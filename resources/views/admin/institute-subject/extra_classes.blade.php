...
<div class="container-fluid">
    {{ Breadcrumbs::render('extra_classes', request()->iacs_id) }}
    @php
    ...
    <div class="card-box">
        <div class="table-responsive">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="width: 15%;">
                            ...
                            <?php $videodata = $extra_class->extra_class_video ? unserialize($extra_class->extra_class_video) :'';  ?>
            ...
            <td scope="row">{{$extra_class->extra_class_number}}</td>
            <td>{{$extra_class->extra_class_name}}</td>
            <td>{{date('d/m/Y', strtotime($extra_class->extra_class_date))}}</td>
            <td><a data-fancybox=""
                class="btn-theme text-white btn-style justify-content-center text-center fancybox-gallery"
                rel="video-gallery" href="{{url($extra_class->extra_class_video)}}" target="_blank">
                <span class="text-video mr-1 text-capitalize text-white">View Extra Class</span> <i
                  class="mdi mdi-play-circle"></i></a></td>
            <td><a class="btn-theme text-white btn-style justify-content-center text-center"
                href="{{url($extra_class->notes??'')}}" target="_blank">Download Notes</a></td>
          </tr> -->
                    @endforeach
                    {{-- <tr>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="card-box">
                    <form role="form" action="{{ url('institute/extra_classes/store') }}" method="post"
...
$(document).ready(function() {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        ...
                        url: "{{ url('admin/extra_classes/delete') }}",
                        method: "post",
                        data: {
                            id: id
                        },
    ...
    $('.editLecture').click(function(e) {
        ...
        $.ajax({
            ...
            url: "{{ url('admin/extra_classes/getLecture') }}",
            method: "post",
            data: {
                id: id
            },
    ...
    $('.videoFile').change(function(e) {
        ...
        jqXHR1 = $.ajax({
            ...
            type: "POST",
            url: "{{ url('admin/extra_classes/updvideo') }}",
            data: data,
            xhr: function() {
            ...
            error: function(_jqXHR, textStatus) {
                ...
                if (errors.errors.extra_class_video) {
                    $this.before(
                        '<br><div class="alert alert-danger alert-dismissable"> <a type="button" class="close" data-dismiss="alert" aria-label="Close"><span aria-hidden="true">&times;</span></a>Video file error : Please select only .mp4, .mov, .ogg, .mpeg, .avi file types for video</div>'
                    );
                }
    ...
    $('.createClicked').click(function(e) {
        ...
        data.append('i_assigned_class_subject_id', '{{request()->iacs_id}}');
        jqXHR = $.ajax({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            type: 'POST',
            url: '{{ url("admin/extra_classes/store") }}',
            data: data,
            xhr: function() {
                var myXhr = $.ajaxSettings.xhr();
                if (myXhr.upload) {
            ...
            error: function(_jqXHR, textStatus) {
                ...
                if (errors.errors.extra_class_video) {
                    $this.before(
                        '<div class="alert alert-danger">Video file error : Please select only .mp4, .mov, .ogg, .mpeg, .avi file types for video</div>'
                    );
                }
                setTimeout(() => {
                    $('.alert-danger').hide(300);
    ...
    var select2 = $('#select2_id').select2({
        ...
        if ($(this).find("option:selected").data("select2-tag") == true) {
            $.ajax({
                url: "{{ url('admin/extra_classes/addUnit') }}",
                method: "post",
                dataType: "json",
                data: {
    ...
    $('#select0_id').on('change', function() {
        $.ajax({
            url: "{{url('admin/extra_classes/getSubjectsByClassId', request()->iacs_id)}}",
            method: 'get',
            dataType: 'json',
            data: {
                id: $(this).val()
    ...
    $('#select0_id, #select1_id').on('change', function() {
        $.ajax({
            url: "{{url('admin/extra_classes/getUnits', request()->iacs_id)}}",
            method: 'get',
            dataType: 'json',
            data: {
