...
<div class="container-fluid">
    {{ Breadcrumbs::render('lectures', request()->i_assigned_class_subject_id) }}
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="width: 15%;">
                            ...
                            <?php $videodata = $lecture->lecture_video ? unserialize($lecture->lecture_video) :'';  ?>
                        ...
                        <td style="width: 15%;">
                            ...
                            <?php $notesData = unserialize($lecture->notes); ?>
                            <a class="btn-theme text-white btn-style justify-content-center text-center"
...
<!-- <div class="loading" style='display:none;'>Uploading... please wait</div>  -->
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="card-box">
                    <!--  <form role="form" class='lectureForm' action="{{ route('institute.lectures.store', request()->i_assigned_class_subject_id) }}"
                ...
                <div class='inputFields'>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="lecture_date" class="form-control" min="{{ date('Y-m-d') }}"
                            id="lecture_date" required>
                    </div>
                ...
                <div class="form-group videoInput" style='display:none;'>
                    ...
                    <input type="file" name="lecture_video" class="form-control dropzone lecture_video"
                        id="lecture_video" accept="video/*">
...
$(document).ready(function() {
    $('.lecture_video').change(function(e) {
        e.preventDefault();
        var file = this.files[0];
        var fileType = file.type;
        var match = ['video/mp4', 'video/mov', 'video/ogg', 'video/mpeg', 'video/avi', 'video/avi'];
        if (!((fileType == match[0]) || (fileType == match[1]) || (fileType == match[2]) || (fileType ==
                match[3]) || (fileType == match[4]) || (fileType == match[5]))) {
            swal("Oppps!", "Sorry, only mp4, mov, ogg, mpeg, avi File  are allowed !", "error");
            $("#file").val('');
            return false;
        }
    })
    ...
    $('.createClicked').click(function(e) {
        ...
        data.append('notes', $('#notes').get(0).files.length > 0 ? $('#notes').get(0).files[0] :
            '');
        data.append('lecture_video', $('#lecture_video').get(0).files.length > 0 ? $(
        ...
        jqXHR = $.ajax({
            ...
            type: 'POST',
            url: "{{ route('institute.lectures.store', request()->i_assigned_class_subject_id) }}",
            data: data,
            xhr: function() {
            ...
            error: function(_jqXHR, textStatus) {
                ...
                if (errors.errors && errors.errors.lecture_video) {
                    $this.before(
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        ...
                        url: "{{ route('institute.lectures.delete', request()->i_assigned_class_subject_id) }}",
                        method: "post",
                        data: {
                            id: id
                        },
    ...
    $('.vide oFile').change(function() {
        $this = $(this);
        $this.parent('.videosDiv').find('.cancleUpload').show();
        $this.parent('.videosDiv').find('.progress').show();
        $this.parent('.videosDiv').find('.percent').show();
        $this.parent('.videosDiv').find('.percent').html('30%');
        $this.parent('.videosDiv').find('.progress').find('.progress-bar').attr('aria-valuemax',
            '30')
        $this.parent('.videosDiv').find('.progress').find('.progress-bar').css('width', '30%')
        $this.parent('.videosDiv').find('.progress').find('.progress-bar').attr('aria-valuenow',
            '30')

        setTimeout(() => {


            $this.parent('.videosDiv').find('.progress')
            $this.hide();
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    ...
                    $this.parent('.videosDiv').find('.percent').html('60%');
                    $this.parent('.videosDiv').find('.progress').find(
                        '.progress-bar').attr('aria-valuemax', '60')
                    $this.parent('.videosDiv').find('.progress').find(
                        '.progress-bar').css('width', '60%')
                    $this.parent('.videosDiv').find('.progress').find(
                        '.progress-bar').attr('aria-valuenow', '60')
                },
                url: "{{ route('institute.lectures.addvideo', request()->i_assigned_class_subject_id) }}",
                method: "post",
                data: data,
                processData: false,
                contentType: false,
                success: function(response) {
                    ...
                    $this.parent('.videosDiv').find('.percent').html('80%');
                    $this.parent('.videosDiv').find('.progress').find(
                        '.progress-bar').attr('aria-valuemax', '80')
                    $this.parent('.videosDiv').find('.progress').find(
                    ...
                    setTimeout(() => {
                        $this.parent('.videosDiv').find('.percent')
                            .html('99%');
                        $this.parent('.videosDiv').find('.progress')
                            .find('.progress-bar').attr('aria-valuemax',
                                '99')
                        $this.parent('.videosDiv').find('.progress')
                            .find('.progress-bar').css('width', '99%')
                        $this.parent('.videosDiv').find('.progress')
                            .find('.progress-bar').attr('aria-valuenow',
                                '99')
                        if (response.status == true) {
                            $this.after(
    ...
    $('.editLecture').click(function(e) {
        ...
        $.ajax({
            ...
            url: "{{ route('institute.lectures.getLecture', request()->i_assigned_class_subject_id) }}",
            method: "post",
            data: {
                id: id
            },
    ...
    /*  $('.cancleButton').click(function(e){
     ...
     $('.cancleUpload').click(function(e){
         e.preventDefault();
         window.location.reload();
     }); */

    /*    var jqXHR;
       $('.create Clicked').click(function(e){
         e.preventDefault();
         var $this = $(this);
         $(".loading").show();
         $(".progress").show();
         ...
         $(".close").hide();
               ...
               data.append('lecture_video',$('#lecture_video').get(0).files.length > 0 ? $('#lecture_video').get(0).files[0] : '');
               ...
               var jqXHR =  $.ajax({
                   ...
                   success: function(response) {
                     setTimeout(() => {
                         ...
                         $('.createClicked').hide();
                         $("#cancleButton").hide();
                         $this.after('<br><span class="alert alert-success text-green">Lecture Created Successfully</span>');
                         setTimeout(() => {
                           $('.progress').hide();
                           window.location.reload();
                         }, 1000);
